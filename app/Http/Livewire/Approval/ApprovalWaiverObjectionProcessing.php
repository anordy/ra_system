...
class ApprovalWaiverObjectionProcessing extends Component
{
    ...
    public $modelName;
    public $comments;
    public $objectionReport;public $taxTypes;
    public $penaltyPercent, $penaltyAmount;
    public $interestPercent, $interestAmount, $waiverObjection, $assesment, $total;
    ...
    public function mount($modelName, $modelId)
    {
        ...
        $this->waiverObjection = WaiverObjection::find($this->modelId);
    ...
    public function updated($propertyName)
    {
        if ($propertyName == "penaltyPercent") {
            ...
            $this->penaltyAmount = ($this->waiverObjection->taxVerificationAssesment->penalty_amount * $this->penaltyPercent) / 100;
        ...
        if ($propertyName == "interestPercent") {
            ...
            $this->interestAmount = ($this->waiverObjection->taxVerificationAssesment->interest_amount * $this->interestPercent) / 100;
        }

        $this->total = $this->interestAmount + $this->penaltyAmount + $this->waiverObjection->taxVerificationAssesment->principal_amount;
    ...
    public function approve($transtion)
    {
        ...
        if ($this->checkTransition('objection_manager_review')) {
            ...
            $waiverObjection = WaiverObjection::find($this->modelId);

            DB::beginTransaction();
            try {

                $waiverObjection->update([
    ...
    try {
        ...
        $billitems = [
            [
                'billable_id' => $this->waiverObjection->id,
                'billable_type' => get_class($this->waiverObjection),
        ...
        $billableId = $this->waiverObjection->id;
        $billableType = get_class($this->waiverObjection);
        ...
        if (config('app.env') != 'local') {
            ...
            if ($response->status === ZmResponse::SUCCESS) {
                $this->waiverObjection->status = ReturnStatus::CN_GENERATING;
                $this->waiverObjection->save();

                $this->flash('success', 'A control number has been generated successful.');
            } else {

                session()->flash('error', 'Control number generation failed, try again later');
                $this->waiverObjection->status = ReturnStatus::CN_GENERATION_FAILED;
            }
            ...
            $this->subject->status = WaiverObjectionStatus::APPROVED;
            $this->subject->save();
