...
class UsersTable extends DataTableComponent
{
            ...
            Column::make('Status', 'status')
                ->label(function ($row) {
                    ...
                    if ($row->id == auth()->user()->id) {
                        ...
                        return <<< HTML
                            <button class="btn btn-info btn-sm" wire:click="activate($id, $row->status)"><i class="fa fa-lock-open"></i> </button>
                        HTML;
                    } else if ($row->status != 1 && Gate::allows('setting-user-change-status')) {
                        return <<< HTML
                            <button class="btn btn-danger btn-sm" wire:click="activate($id, $row->status)"><i class="fa fa-lock"></i> </button>
                        HTML;
                    }
                })
                ->html(true),
            ...
            Column::make('Action', 'id')
                ->format(function ($value, $row) {
                    ...
                    if (Gate::allows('setting-user-change-password')) {
                        $changePwd = <<< HTML
                                    <button class="btn btn-warning btn-sm" onclick="Livewire.emit('showModal', 'user-change-password-modal',$id)"><i class="fa fa-key"></i> </button>
                                HTML;
                    }
                    ...
                    if ($row->is_first_login == 1) {
                        $mail = <<< HTML
                            <button class="btn btn-secondary btn-sm" wire:click="resendCredential($id)"><i class="fa fa-envelope"></i> </button>
                        HTML;
                    }
                ...
                ->format(function ($value, $row) {
                    ...
                    if ($row->is_approved == 1) {
                        if (Gate::allows('setting-user-change-role')) {
                            return <<< HTML
                                        <button class="btn btn-success btn-sm" onclick="Livewire.emit('showModal', 'user-role-edit-modal', $id)"><i class="fa fa-user-tag"></i> </button>
                                    HTML;
                        }
                    }
                })
    ...
    public function toggleStatus($value)
    {
        ...
        try {
            ...
            if (empty($user)) {
                $this->alert('error', 'The selected user is not found');
                return;
            }
