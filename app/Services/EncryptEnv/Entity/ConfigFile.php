...
namespace App\Services\EncryptEnv\Entity;
...
class ConfigFile
{
    ...
    {
        if (file_exists(config_path('envsecure.php'))) {
            $encenv_config = require(config_path('envsecure.php'));
        ...
        $this->configarr = !empty($this->configpath) && file_exists($this->configpath) ? require $this->configpath : (!empty($_ENV) ? $_ENV : getenv());
