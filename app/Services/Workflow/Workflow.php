...
use App\Services\Workflow\Exception\NotEnabledTransitionException;
use App\Services\Workflow\Exception\UndefinedTransitionException;
...
class Workflow implements WorkflowInterface
{
    ...
    public function getMarking(object $subject, array $context = [])
    {
        ...
        if (!$marking->getPlaces()) {
            ...
            foreach ($this->definition->getInitialPlaces() as $key => $place) {
                $marking->mark($key, (array) $place);
            }
    ...
    private function enter(object $subject, Transition $transition, Marking $marking, array $context): void
    {
        ...
        foreach ($places as $placeName) {
            $place =(array) $this->getDefinition()->getPlaces()[$placeName];
            $marking->mark($placeName, $place);
        }
    }
