...
class ZmFeeHelper
{
    ...
    public static function getTransactionFee(array $billItems, $currency, $exchangeRate)
    {
        if (!is_numeric($exchangeRate) || $exchangeRate <= 0) {
            throw new \Exception("Exchange rate can not be zero, null or not a number.");
        }
        ...
        return [
            ...
            'amount' => round($fee, 2),
            'currency' => $currency,
            'gfs_code' => TaxType::where('code', TaxType::GOVERNMENT_FEE)->firstOrFail()->gfs_code,
            'tax_type_id' => TaxType::where('code', TaxType::GOVERNMENT_FEE)->firstOrFail()->id,
        ];
    }
}
