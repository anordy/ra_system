<?php

namespace Database\Seeders;

use App\Models\PortConfig;
use App\Models\TaxType;
use Illuminate\Database\Seeder;

class PortConfigSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        $configs = [
            [
                'financia_year_id' => 1,
                'order' => 1,
                'tax_type_code' => TaxType::AIRPORT_SERVICE_SAFETY_FEE,
                'code' => 'NFAT',
                'name' => 'No. of foreign passengers (Airport Tax)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'USD',
                'rate_usd' => 40,
                'active' => true,
            ],
            [
                'financia_year_id' => 1,
                'order' => 2,
                'tax_type_code' => TaxType::AIRPORT_SERVICE_SAFETY_FEE,
                'code' => 'NLAT',
                'name' => 'No. of local passengers (Airport Tax)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'TZS',
                'rate' => 10000,
                'active' => true,
            ],
            [
                'financia_year_id' => 1,
                'order' => 3,
                'tax_type_code' => TaxType::AIRPORT_SERVICE_SAFETY_FEE,
                'code' => 'NFSF',
                'name' => 'No. of Foreign passengers (Safety Fee)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'USD',
                'rate_usd' => 9,
                'active' => true,
                'value_calculated' => true,
                'value_formular' => 'NFAT',
            ],
            [
                'financia_year_id' => 1,
                'order' => 4,
                'tax_type_code' => TaxType::AIRPORT_SERVICE_SAFETY_FEE,
                'code' => 'NLSF',
                'name' => 'No. of local passengers (Safety fee)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'TZS',
                'rate' => 3000,
                'active' => true,
                'value_calculated' => true,
                'value_formular' => 'NLAT',
            ],
            [
                'financia_year_id' => 1,
                'order' => 5,
                'tax_type_code' => TaxType::AIRPORT_SERVICE_SAFETY_FEE,
                'code' => 'IT',
                'name' => 'Infrastructure Tax',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'TZS',
                'rate' => 2000,
                'active' => true,
                'value_calculated' => true,
                'value_formular' => 'NLAT',
            ],
            [
                'financia_year_id' => 1,
                'order' => 6,
                'tax_type_code' => TaxType::AIRPORT_SERVICE_SAFETY_FEE,
                'code' => 'TLATZS',
                'name' => 'Total Amount Fee Due(TZS)',
                'row_type' => 'unremovable',
                'col_type' => 'total',
                'formular' => 'NLAT+NLSF+IT',
                'active' => true,
            ],
            [
                'financia_year_id' => 1,
                'order' => 7,
                'tax_type_code' => TaxType::AIRPORT_SERVICE_SAFETY_FEE,
                'code' => 'TLAUSD',
                'name' => 'Total Amount Fee Due (US$)',
                'row_type' => 'unremovable',
                'col_type' => 'total',
                'formular' => 'NFAT+NFSF',
                'active' => true,
            ],
            //   Sea Port & Transport Tax
            [
                'financia_year_id' => 1,
                'order' => 8,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'NFSP',
                'name' => 'No. of foreign passengers',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'USD',
                'rate_usd' => 15,
                'active' => true,
            ],
            [
                'financia_year_id' => 1,
                'order' => 9,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'NLTM',
                'name' => 'No. of local passengers(ZNZ - T/M)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'TZS',
                'rate' => 2000,
                'active' => true,
            ],
            [
                'financia_year_id' => 1,
                'order' => 9,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'NLNRTM',
                'name' => 'No. of non-resident local passengers(ZNZ - T/M)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'USD',
                'rate_usd' => 2,
                'active' => true,
            ],
            [
                'financia_year_id' => 1,
                'order' => 12,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'ITTM',
                'name' => 'Infrastructure Tax (ZNZ - T/M)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'TZS',
                'rate' => 2000,
                'active' => true,
                'value_calculated' => true,
                'value_formular' => 'NLTM',
            ],
            [
                'financia_year_id' => 1,
                'order' => 12,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'ITNRTM',
                'name' => 'Infrastructure Tax Non-Resident (ZNZ - T/M)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'USD',
                'rate_usd' => 2,
                'active' => true,
                'value_calculated' => true,
                'value_formular' => 'NLNRTM',
            ],
            [
                'financia_year_id' => 1,
                'order' => 10,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'NLZNZ',
                'name' => 'No. of local passengers (ZNZ - ZNZ)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'TZS',
                'rate' => 1000,
                'active' => true,
            ],
            [
                'financia_year_id' => 1,
                'order' => 10,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'NLNRZNZ',
                'name' => 'No. of non resident passengers (ZNZ - ZNZ)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'USD',
                'rate_usd' => 1,
                'active' => true,
            ],
            [
                'financia_year_id' => 1,
                'order' => 11,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'ITZNZ',
                'name' => 'Infrastructure Tax (ZNZ - ZNZ)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'TZS',
                'rate' => 1000,
                'rate_usd' => 0,
                'active' => true,
                'value_calculated' => true,
                'value_formular' => 'NLZNZ',
            ],
            [
                'financia_year_id' => 1,
                'order' => 11,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'ITNRZNZ',
                'name' => 'Infrastructure Tax Non Resident (ZNZ - ZNZ)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'fixed',
                'currency' => 'USD',
                'rate' => 1,
                'rate_usd' => 1,
                'active' => true,
                'value_calculated' => true,
                'value_formular' => 'NLNRZNZ',
            ],

            [
                'financia_year_id' => 1,
                'order' => 13,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'NSUS',
                'name' => 'Value of Net Sales (US$)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'percentage',
                'currency' => 'USD',
                'rate' => 8,
                'active' => true,
            ],
            [
                'financia_year_id' => 1,
                'order' => 15,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'CSUS',
                'name' => 'Cargo value of Net Sales (US$)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'percentage',
                'currency' => 'USD',
                'rate' => 8,
                'active' => true,
            ],
            [
                'financia_year_id' => 1,
                'order' => 18,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'TLSTZS',
                'name' => 'Total Amount Fee Due(TZS)',
                'row_type' => 'unremovable',
                'col_type' => 'total',
                'formular' => 'NLTM+NLZNZ+ITZNZ+ITTM+NSTZ+CSUS',
                'active' => true,
            ],
            [
                'financia_year_id' => 1,
                'order' => 14,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'NSTZ',
                'name' => 'Value of Net Sales (TZS)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'percentage',
                'currency' => 'TZS',
                'rate' => 8,
                'active' => true,
            ],
            [
                'financia_year_id' => 1,
                'order' => 17,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'TLSUSD',
                'name' => 'Total Amount Fee Due (US$)',
                'row_type' => 'unremovable',
                'col_type' => 'total',
                'formular' => 'NFSP+NSUS+CSUS+NLNRTM+NLNRZNZ+ITNRTM+ITNRZNZ',
                'active' => true,
            ],
            [
                'financia_year_id' => 1,
                'order' => 16,
                'tax_type_code' => TaxType::SEAPORT_SERVICE_TRANSPORT_CHARGE,
                'code' => 'CSTZ',
                'name' => 'Cargo value of Net Sales (TZS)',
                'row_type' => 'dynamic',
                'col_type' => 'normal',
                'rate_applicable' => true,
                'rate_type' => 'percentage',
                'currency' => 'TZS',
                'rate' => 8,
                'active' => true,
            ],
        ];

        foreach ($configs as $config) {
            PortConfig::updateOrCreate([
                'code' => $config['code']
            ],$config);
        }
    }
}
