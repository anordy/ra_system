@if (count($this->getEnabledTransitions()) > 1)
    <div class="card shadow-sm mb-2 bg-white border-0 shadow-none">
        <div class="card-header font-weight-bold">
            Transport Service Temporary Closure Approval
        </div>
        <div class="card-body">
        ...
        @if ($this->checkTransition('public_service_registration_officer_review'))
            <div class="modal-footer p-2 m-0">
                ...
                <button type="button" class="btn btn-primary" wire:click="confirmPopUpModal('approve', 'public_service_registration_officer_review')">
                    Approve & Forward
                </button>
            </div>
        @elseif ($this->checkTransition('public_service_registration_manager_review'))
            <div class="modal-footer p-2 m-0">
                <button type="button" class="btn btn-danger" wire:click="confirmPopUpModal('reject', 'public_service_registration_manager_reject')">
                    Reject & Return
                </button>
                <button type="button" class="btn btn-primary" wire:click="confirmPopUpModal('approve', 'public_service_registration_manager_review')">
                    Approve & Complete
                </button>
            </div>
        @endif
