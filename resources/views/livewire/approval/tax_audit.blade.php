            ...
            @if ($this->checkTransition("audit_date_extension_dc_review"))
                @if ($this->subject->new_audit_date)
                    <p>{{ __("Tax Payer Requested Audit Extension Until ") }}
                        <strong
                            class="text-secondary">{{ $this->subject->new_audit_date ? \Carbon\Carbon::parse($this->subject->new_audit_date)->format("F j, Y") : "" }}
                        </strong>
                    </p>

                    <p> <strong> Extension Reasons:</strong> <br> {{ $this->subject->extension_reason }}</p>
                    <p> Extension Attachment: </p>
                    <div class="col-md-3">
                        <div style="background: #faf5f5; color: #036a9e; border: .5px solid #036a9e24;"
                            class="p-2 mb-3 d-flex rounded-sm align-items-center">
                            <i class="bi bi-file-earmark-pdf-fill px-2" style="font-size: x-large"></i>
                            <a target="_blank"
                                href="{{ route("tax_auditing.files.show", encrypt($this->subject->audit_date_extension_attachment)) }}"
                                style="font-weight: 500;" class="ml-1">
                                Attachment
                                <i class="bi bi-arrow-up-right-square ml-1"></i>
                            </a>
                    ...
                    <div class="col-6">
                        <div class="d-flex justify-content-between ">
                            <div class="form-check">
                                ...
                                <label class="form-check-label" for="accept"> {{ __("Accept") }} </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="acceptance" id="accept" value="reject"
                                    wire:model="selectedOption">
                                <label class="form-check-label" for="reject"> {{ __("Reject") }} </label>
                            </div>

                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="acceptance" id="changed" value="changed"
                                    wire:model="selectedOption">
                                <label class="form-check-label" for="changed"> {{ __("Change Date") }} </label>
                            </div>
                        </div>
                    ...
                    @if ($selectedOption == "changed")
                        <div class="row d-flex justify-content-between mt-3">
                            <div class="col-6 ">
                                <label for="new_audit_date">{{ __("New Audit Date") }}:</label>
                                <input type="date" id="new_audit_date" value="{{ $newAuditDate }}" wire:model.defer="newAuditDate"
                                    class="form-control"
                                    placeholder="{{ __("Enter New Audit Date") }}">
                                @error("newAuditDate")
