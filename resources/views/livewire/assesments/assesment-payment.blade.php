@if ($bill = $assessment->bill)
    <div class="row py-4 alert alert-secondary bg-alt rounded-0 shadow-sm border-success">
        ...
        @endif
        @if ($assessment->payment_status === \App\Models\Returns\ReturnStatus::CN_GENERATED ||
            $assessment->payment_status === \App\Models\Returns\ReturnStatus::PAID_PARTIALLY)
            @if ($bill->zan_trx_sts_code == \App\Services\ZanMalipo\ZmResponse::SUCCESS || $bill->zan_trx_sts_code == \App\Services\ZanMalipo\ZmResponse::ZM_DUPLICATE_BILL_INFO)
            <div class="col-md-3">
        ...
        @elseif($assessment->payment_status === \App\Models\Returns\ReturnStatus::CN_GENERATING)
...
<div class="row py-4 alert alert-secondary bg-alt rounded-0 shadow-sm border-success">
    <div class="col-md-3">
        ...
        <p class="my-1">{{ number_format($assessment->total_amount, 2) }} {{ $assessment->currency }}
        </p>
    </div>
    <div class="col-md-3">
        <span class="font-weight-bold text-uppercase">Bill Status</span>
