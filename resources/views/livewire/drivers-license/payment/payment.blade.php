    ...
    @if (!empty($license->get_latest_bill()))
        <div class="row py-4 alert alert-secondary bg-alt rounded-0 shadow-sm border-success">
            <div class="col-md-4">
                ...
                <p class="my-1">{{ number_format($license->get_latest_bill()->amount, 2) }}
                    {{ $license->get_latest_bill()->currency }}</p>
            </div>
            @if (
                $license->payment_status === \App\Models\Returns\ReturnStatus::CN_GENERATED ||
                    $license->payment_status === \App\Models\Returns\ReturnStatus::PAID_PARTIALLY)

                @if ($license->get_latest_bill()->zan_trx_sts_code == \App\Services\ZanMalipo\ZmResponse::SUCCESS)
                    <div class="col-md-4">
                        <span class="font-weight-bold text-uppercase">Control No.</span>
                        <p class="my-1">{{ $license->get_latest_bill()->control_number }}</p>
                    </div>
                    <div class="col-md-4">
                        <span class="font-weight-bold text-uppercase"> </span>
                        <p class="my-1">
                            <a target="_blank"
                                href="{{ route('bill.invoice', encrypt($license->get_latest_bill()->id)) }}"
                ...
                @endif
            @elseif($license->payment_status === \App\Models\Returns\ReturnStatus::COMPLETE)
                <div class="col-md-4">
                ...
                @if (get_class($license) != 'App\Models\MvrRegistrationStatusChange')
                    <div class="col-md-3">
                        <span class="font-weight-bold text-uppercase"> </span>
                        <p class="my-1">
                            <a target="_blank" href="{{ route('mvr.registration.certificate', encrypt($license->id)) }}"
            ...
            @elseif($license->payment_status === \App\Models\Returns\ReturnStatus::CN_GENERATING)
            ...
            @elseif($license->payment_status === \App\Models\Returns\ReturnStatus::CN_GENERATION_FAILED)
                <div class="col-md-4">
                    <span class="font-weight-bold text-uppercase">Control No. Generation Failed</span>
                    <p class="my-1 text-danger">
                        Generation Failed
                    ...
                    <p class="my-1">
                        <button target="_blank" wire:click="regenerate"
                            ...
                            <i class="bi bi-arrow-repeat mr-2" wire:loading.remove
                                wire:target="regenerate"></i>Regenerate
                            Control No
                        </button>
                    </p>
                </div>
            ...
            <div class="col-md-12 mt-3">
                ...
                <span>
                    {{ $this->getGepgStatus($license->get_latest_bill()->zan_trx_sts_code) }}
                </span>
            </div>
        </div>
    @else
