<div class="px-3">
    @if(!empty($deregistration->latestBill))
        <div class="row py-4 alert alert-secondary bg-alt rounded-0 shadow-sm border-success">
            <div class="col-md-3">
            <span class="font-weight-bold text-uppercase">
               Total Fee Paid
            </span>
                <p class="my-1">{{ number_format($deregistration->latestBill->amount, 2) }} {{ $deregistration->latestBill->currency }}</p>
            </div>
            @if ($deregistration->payment_status === \App\Models\Returns\ReturnStatus::CN_GENERATED ||
                $deregistration->payment_status === \App\Models\Returns\ReturnStatus::PAID_PARTIALLY)

                @if($deregistration->latestBill->zan_trx_sts_code == \App\Services\ZanMalipo\ZmResponse::SUCCESS)
                    <div class="col-md-4">
                        <span class="font-weight-bold text-uppercase">Control No.</span>
                        <p class="my-1">{{ $deregistration->latestBill->control_number }}</p>
                    </div>
                    <div class="col-md-4">
                        <span class="font-weight-bold text-uppercase"> </span>
                        <p class="my-1">
                            <a target="_blank"
                               href="{{ route('bill.invoice', encrypt($deregistration->latestBill->id)) }}"
                ...
                @endif
            @elseif($deregistration->payment_status === \App\Models\Returns\ReturnStatus::COMPLETE)
                <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <p class="my-1">
                            <a target="_blank"
                               href="{{ route('mvr.de-registration.certificate', encrypt($deregistration->id)) }}"
                               class="btn btn-primary btn-sm pl-3 pr-4 font-weight-bold">
                                <i class="bi bi-download mr-3"></i><u>{{ __('Download Certificate') }}</u>
                            </a>
                        </p>
                    </div>
                <div class="col-md-3">
                    <span class="font-weight-bold text-uppercase"> </span>
                    <p class="my-1">
                        <a target="_blank" href="{{ route('bill.receipt', encrypt($deregistration->latestBill->id)) }}"
                           class="btn btn-primary btn-sm pl-3 pr-4 font-weight-bold">
            ...
            @elseif($deregistration->payment_status === \App\Models\Returns\ReturnStatus::CN_GENERATING)
            ...
            @elseif($deregistration->payment_status === \App\Models\Returns\ReturnStatus::CN_GENERATION_FAILED)
                <div class="col-md-4">
                    <span class="font-weight-bold text-uppercase">Control No. Generation Failed</span>
                    <p class="my-1 text-danger">
                        Generation Failed
            ...
            <div class="col-md-12 mt-3">
                ...
                <span>
                {{ $this->getGepgStatus($deregistration->latestBill->zan_trx_sts_code) }}
            </span>
            </div>
        </div>
    @else
