<div>
    @if($viableFinancialYear)
        <div class="alert alert-warning rounded-0 shadow-sm mx-n3 border-warning" role="alert">
            Your property is viable to generate control number for the year <strong>{{ $viableFinancialYear->code ?? 'N/A' }}</strong>, Please click the button on the right to Generate Control number.
            <button type="button" wire:click="generateFinancialYearPayment()" class="btn btn-sm btn-primary float-right">
                Click Here to Generate Control Number
            </button>
        </div>
    @endif
