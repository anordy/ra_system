...
@elseif($bill = $return->debt->bill)
    <div class="row py-4 alert alert-secondary bg-alt rounded-0 shadow-sm border-success">
        <div class="col-md-4">
            <span class="font-weight-bold text-uppercase">Total Payment Amount</span>
            @if ($bill)
                <p class="my-1">{{ number_format($bill->amount, 2) }} TZS</p>
            @endif
        </div>
        @if ($return->debt->status === \App\Models\Returns\ReturnStatus::CN_GENERATED ||
            $return->debt->status === \App\Models\Returns\ReturnStatus::PAID_PARTIALLY)
            ...
            @if($bill->zan_trx_sts_code == \App\Services\ZanMalipo\ZmResponse::SUCCESS)
                ...
                <div class="col-md-4">
                    <span class="font-weight-bold text-uppercase"> </span>
                    <p class="my-1">
                        <a target="_blank" href="{{ route('bill.invoice', encrypt($bill->id)) }}" class="btn btn-primary btn-sm pl-3 pr-4 font-weight-bold">
                            <i class="bi bi-download mr-3"></i><u>Download Bill</u>
                        </a>
                    </p>
                </div>
            ...
            @endif
        @elseif($return->debt->status === \App\Models\Returns\ReturnStatus::COMPLETE)
            <div class="col-md-4">
                <span class="font-weight-bold text-uppercase">Payment Status</span>
                <p class="my-1 text-success font-weight-bold">
                    <i class="bi bi-check-circle-fill mr-2"></i>
        ...
        @elseif($return->debt->status === \App\Models\Returns\ReturnStatus::CN_GENERATING)
            <div class="col-md-4" wire:poll.visible="refresh">
                <span class="font-weight-bold text-uppercase text-info">Control No.</span>
                <p class="my-1 text-info">
                    <i class="bi bi-clock-history mr-2"></i>
        ...
        @elseif($return->debt->status === \App\Models\Returns\ReturnStatus::CN_GENERATION_FAILED)
            <div class="col-md-4">
                <span class="font-weight-bold text-uppercase">Control No. Generation Failed</span>
                <p class="my-1 text-danger">
                    Generation Failed
