@if(!empty($taxRefund->latestBill))
    <div class="row py-4 alert alert-secondary bg-alt rounded-0 shadow-sm border-success">
        <div class="col-md-4">
            ...
            <p class="my-1">{{ number_format($taxRefund->latestBill->amount, 2) }} {{ $taxRefund->latestBill->currency }}</p>
        </div>
        @if ($taxRefund->payment_status === \App\Models\Returns\ReturnStatus::CN_GENERATED ||
            $taxRefund->payment_status === \App\Models\Returns\ReturnStatus::PAID_PARTIALLY)
            @if($taxRefund->latestBill->zan_trx_sts_code == \App\Services\ZanMalipo\ZmResponse::SUCCESS)
                <div class="col-md-4" wire:poll.visible.30000ms="refresh" wire:poll.30000ms>
                    ...
                    <p class="my-1">{{ $taxRefund->latestBill->control_number }}</p>
                </div>
                <div class="col-md-4">
                    <span class="font-weight-bold text-uppercase"> </span>
                    <p class="my-1">
                        <a target="_blank" href="{{ route('bill.invoice', encrypt($taxRefund->latestBill->id)) }}"
            ...
            @endif
        @elseif($taxRefund->payment_status === \App\Models\Returns\ReturnStatus::COMPLETE)
            <div class="col-md-4">
        ...
        <div class="col-md-12 mt-3">
            ...
            <span>
                {{ $this->getGepgStatus($taxRefund->latestBill->zan_trx_sts_code) }}
            </span>
        </div>
    </div>
@else
    <div class="row py-4 alert alert-secondary bg-alt rounded-0 shadow-sm border-success">
            ...
            <span class="font-weight-bold text-uppercase">
                ...
                <p class="my-1">{{ number_format($taxRefund->total_payable_amount, 2) }} {{ $taxRefund->currency }}
                </p>
            </div>
        @if ($taxRefund->total_payable_amount > 0)
            <div class="col-md-3">
