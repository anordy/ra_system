        ...
        <div id="tab1" class="tab-pane fade active show m-2">
            ...
            <div class="row m-4 pt-3">
                <div class="col-md-4 mb-3">
                    ...
                    <p class="my-1">
                        @if($withholding_agent->app_status === \App\Models\WithholdingAgentStatus::APPROVED)
                            <span class="font-weight-bold text-success">
                        <i class="bi bi-check-circle-fill mr-1"></i>
                        {{ __('Approved') }}
                    </span>
                    @elseif($withholding_agent->app_status === \App\Models\WithholdingAgentStatus::REJECTED)
                    <span class="font-weight-bold text-danger">
                        <i class="bi bi-check-circle-fill mr-1"></i>
                        {{ __('Rejected') }}
                    </span>
                    @elseif($withholding_agent->app_status === \App\Models\WithholdingAgentStatus::CORRECTION)
                    <span class="font-weight-bold text-warning">
                        <i class="bi bi-pen-fill mr-1"></i>
                        {{ __('Requires Correction') }}
                    </span>
                        @else
            ...
            <livewire:approval.withholding-agent-approval-processing modelName='App\Models\WithholdingAgent'
            modelId="{{ encrypt($withholding_agent->id) }}" />
        </div>
        <div id="tab2" class="tab-pane fade m-2">
            <livewire:approval.approval-history-table modelName='App\Models\WithholdingAgent'
            modelId="{{ encrypt($withholding_agent->id) }}" />
        </div>
    </div>
