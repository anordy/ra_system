...
@section('title', 'Reversal Details')
...
@section('content')
        ...
        <div class="card-body p-2">
            <div class="row ">
                <div class="col-md-3 mb-3">
                    ...
                    <p class="my-1 text-uppercase">{{ $payment->amount ?? 'N/A' }} {{ $payment->currency }}</p>
                </div>
                <div class="col-md-3 mb-3">
                    <span class="font-weight-bold text-uppercase">Transaction Ref</span>
                    <p class="my-1 text-uppercase">{{ $payment->bank_ref ?? 'N/A' }}</p>
                </div>
                <div class="col-md-3 mb-3">
                    <span class="font-weight-bold text-uppercase">Paid At</span>
                    <p class="my-1 text-uppercase">{{ $payment->transaction_time?->toDayDateTimeString() ?? 'N/A' }}</p>
                </div>
                <div class="col-md-3 mb-3">
                    <span class="font-weight-bold text-uppercase">Control No.</span>
                    <p class="my-1 text-uppercase">{{ $payment->control_number ?? 'N/A' }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    @if($bill = $payment->bill)
        ...
        @if($payment->bill->bill_payments->count())
            <div class="card rounded-0">
                <div class="card-header bg-white font-weight-bold text-uppercase">
                    Payment Details
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-12 table-responsive ">
                            <table class="table table-sm  table-bordered ">
                                ...
                                <tbody>
                                @foreach ($payment->bill->bill_payments as $payment)
                                    <tr>
